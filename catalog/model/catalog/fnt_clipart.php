...
class ModelCatalogFntClipart extends Model {
	public function addclipart($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "fnt_clipart SET name = '" . $this->db->escape($data['name']) . "', image = '" . $this->db->escape($data['image']) . "', price = '" . (float)$data['price'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
	...
	public function editclipart($clipart_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "fnt_clipart SET name = '" . $this->db->escape($data['name']) . "', image = '" . $this->db->escape($data['image']) . "', price = '" . (float)$data['price'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "' WHERE clipart_id = '" . (int)$clipart_id . "'");
	...
	public function getClipartByName($category_clipart) {
		$category_clipart_id = $this->db->query("SELECT category_clipart_id FROM " . DB_PREFIX . "fnt_category_clipart_description WHERE name = '" . $this->db->escape($category_clipart) . "'")->row['category_clipart_id'];
		$clipart_data = array();
