...
;(function($) {
	...
	var FancyProductDesigner = function( elem, args) {
		...
		var _init = function() {
					...
					if(elemParams.boundingBox && !options.editorMode) {
						...
						if(typeof elemParams.boundingBox == "object") {
							currentBoundingObject = new fabric.Rect({
								...
								selectable: false,
								isBoundingRect: true,
								originX: elemParams.boundingBox.x,
								originY: elemParams.boundingBox.y
							});
			...
			if(options.facebookAppId && options.facebookAppId.length > 0) {
				...
				var $fbUserPhotos = $sidebarContent.find('.fpd-fb-user-photos'),
				...
				$sidebarNavi.find('li[data-target=".fpd-user-photos"]').show();
				$sidebarNavi.find('.fpd-navigation-upload li[data-target=".fpd-fb-user-photos"]').show();
	...
	$.fn.fancyProductDesigner.defaults = {
		...
		elementParameters: {
			...
			z: true //Allows to set the z-index of an element
