/*
 * Fancy Product Designer v2.1.31
 *
...
;(function($) {
	...
	var FancyProductDesigner = function( elem, args) {
		...
		$productsIdeas = $elem.children('.fpd-product-ideas').remove();
		...
		//Code OPC
			$.post(options.templatesDirectory+'canvaserror', function(html) {
		...
		//load sidebar html
		//Code OPC
		$.post(options.templatesDirectory+'sidebar'+layoutAffix,
			...
			function(html){
				...
				//Code OPC
				$.post(options.templatesDirectory+'productstage'+layoutAffix,
					...
					function(html){
						...
						$sidebarNaviProduct = $elem.find('.fpd-navigation-product');
						...
						if(options.layout != 'semantic') {
							...
							if($elem.hasClass('fpd-horizontal')) {
								...
								$sidebarNaviProduct.find('.fpd-nav-item').css('line-height', options.dimensions.sidebarNavSize + 'px');
								$sidebarNaviProduct.css('height', options.dimensions.sidebarNavSize);
							}
							//flat vertical layout
								...
								$sidebarNaviProduct.css('width', options.dimensions.sidebarNavSize);
						...
						$productContainer = $elem.children('.fpd-product-container').width(options.dimensions.productStageWidth);
						...
						$productLoader = $productContainer.append('<div class="fpd-product-loader"><div class="fpd-loading"></div></div>').children('.fpd-product-loader').hide();
		...
		var _init = function() {
			...
			for(var i=0; i < $products.length; ++i) {
				...
				views.each(function(i, view) {
					var $view = $(view);
					var viewObj = {title: view.title, thumbnail: $view.data('thumbnail'), elements: [],id: views.attr('data-parameters')};
				...
				thisClass.addProduct(viewsArr,'element-product-design');
			}
			if($productsIdeas.length == 0){
				$sidebarNaviProduct.css('display','none');
			}
			for(var i=0; i < $productsIdeas.length; ++i) {
				//get other views
				views = $($productsIdeas.get(i)).children('.fpd-product');
				//get first view
				views.splice(0,0,$productsIdeas.get(i));
				...
				thisClass.addProduct(viewsArr,'element-product-ideas');
			}
			
			$sidebarContent.find('.fpd-products .fpd-items-wrapper .element-product-ideas').parent().css('display','none');
			//load all designs
			if($designs.size() > 0) {
				...
				if(window.FileReader) {
			    	...
			    	reader.onload = function (evt) {
			    		...
			    		image.onload = function() {
				    		...
				    		thisClass.addDesign(evt.target.result, designTitle, imageParams, options.labels.myUploadedImgCat);
			...
			// handleEvent tab Facebook and Instagram
			$sidebar.find('.fpd-user-photos .fpd-navigation-upload .fpd-nav-item').click(function(evt) {
				...
				if(!$this.hasClass('active')) {
					...
					$this.parent().next().find($this.attr('data-target')).css('display','inline-block');
			...
			if(options.facebookAppId && options.facebookAppId.length > 0) {
				...
				$sidebarNavi.find('[data-target=".fpd-user-photos"]').css('display', 'inline-block');
				$('[data-target=".fpd-fb-user-photos"]').css('display', 'inline-block');
				$('[data-target=".fpd-fb-user-photos"]').click();
			...
			if(options.instagramClientId.length) {
				$sidebarNavi.find('[data-target=".fpd-user-photos"]').css('display', 'inline-block');
				$('[data-target=".fpd-instagram-photos"]').css('display', 'inline-block');
				if(options.facebookAppId.length == 0) {
					$('[data-target=".fpd-instagram-photos"]').click();
				}
				$sidebarContent.find('.fpd-instagram-photos').find('.fpd-insta-feed, .fpd-insta-recent-images').click(function(evt) {
			...
			if(options.allowProductSaving && $elem.attr('id')) {
				...
				$menubar.find('.fpd-save-product').click(function(evt) {
					...
					//save price, Currency, product design id
					var product_design_id = $('html').find('input[name=\'product_design_id\']').val();
					var product_price = parseFloat($('html').find('input[name=\'product_price\']').val()) - parseFloat($('html').find('input[name=\'product_price_base\']').val());
					var currency = $('html').find('input[name=\'currency\']').val();
					
					savedProducts.push({thumbnail: thumbnail, product: product, product_design_id: product_design_id, product_price: product_price, currency: currency});
					window.localStorage.setItem($elem.attr('id'), JSON.stringify(savedProducts));

					_addSavedProduct(thumbnail, product, product_design_id, product_price, currency);
				...
				if(savedProducts != null) {
					for(var i=0; i < savedProducts.length; ++i) {
						...
						_addSavedProduct(savedProduct.thumbnail, savedProduct.product, savedProduct.product_design_id, savedProduct.product_price, savedProduct.currency);
					}
				}
			...
			if(options.editorMode) {
				$.post(
				//Code OPC
					options.templatesDirectory+'editorbox',
					function(html){
			...
			$sidebarNaviProduct.find('.fpd-nav-item').click(function(evt) {
				...
				if(!$this.hasClass('active')) {
					$this.parent().children().removeClass('fpd-secondary-bg-color fpd-primary-text-color active').addClass('fpd-secondary-text-color');
					$this.removeClass('fpd-secondary-text-color').addClass('fpd-secondary-bg-color fpd-primary-text-color active');
					$this.parent().parent().find('picture').css('display','none');
					$this.parent().parent().find($this.attr('data-target')).parent().css('display','inline-block');
		...
		var _centerObject = function(object, hCenter, vCenter, boundingBox) {
			...
			if(hCenter) {
				...
				if(boundingBox) {
					...
					if(object.originX == 'left') {
						left = options.dimensions.productStageWidth * 0.5 - object.width * 0.5;
					}
					else {
						left = options.dimensions.productStageWidth * 0.5;
					}
			...
			if(vCenter) {
				if(boundingBox) {
					...
					if(object.originX == 'left') {
						top = options.dimensions.productStageHeight * 0.5 - object.height * 0.5;
					}
					else {
						top = options.dimensions.productStageHeight * 0.5;
					}
		...
		var _addSavedProduct = function(thumbnail, product, product_design_id, product_price, currency) {
			...
			$list.find('.menu').append('<div class="item fpd-clearfix"><img data-parameter="'+product_design_id+':'+product_price+':'+currency+'" src="'+thumbnail+'" /><i class="fpd-primary-text-color fa fa-times"></i></div>')
		...
		var _loadInstaImages = function(endpoint) {
			...
			var $instaPhotosList = $sidebarContent.find('.fpd-instagram-photos').children('.fpd-items-wrapper').css('visibility', 'hidden'),
				$instaLoadNext = $sidebarContent.find('.fpd-instagram-photos').find('.fpd-insta-load-next');
		...
		*
		*/
		this.addProduct = function(views,type) {

			//load product by click
			var $productItemsWrapper = $sidebarContent.find('.fpd-products .fpd-items-wrapper');
			$productItemsWrapper.append('<picture class="'+views[0].id+'"><span class="fpd-loading"></span><img class="'+type+'" src="'+views[0].thumbnail+'" title="'+views[0].title+'" style="display:none;" /></picture>').children('picture:last').click(function(evt) {
				evt.preventDefault();
				var $this = $(this),
