...
class ModelCatalogFntCategory extends Model {
	public function addCategory($data) {
		...
		if ($data['keyword']) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'category_design_id=" . (int)$category_design_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}
	...
	public function getCategory($category_design_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'category_design_id=" . (int)$category_design_id . "') AS keyword FROM " . DB_PREFIX . "fnt_category_design  WHERE category_design_id = '" . (int)$category_design_id . "'");
	...
	public function getCategories($data) {
		...
		$sql .= " AND status = 1 GROUP BY c.category_design_id ORDER BY cd2.name";
